...
var filterWarnings = /^npm WARN (skippingAction|notsup|optional|prefer|deprecated|install) /;

var executeHandler = function (cmds, done) {
  var proj = null;
  var iterator = function (cmd, cb) {
    if (cmd.indexOf(utils.getCommentStart()) == 0) {
      proj = cmd.substring(utils.getCommentStart().length).trim();
      console.log('');
    } else {
    ...
    cp.exec(cmd, function(err, stdout, stderr) {
      if (stderr) {
        var filterOwn = new RegExp('npm WARN ((EPEERINVALID|EPACKAGEJSON) )?' + proj);
        stderr.split('\n').filter(function(l) {
